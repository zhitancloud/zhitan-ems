...
public class ComprehensiveStatisticsController extends BaseController {
    ...
    public AjaxResult getEnergyList(ComprehensiveStatistics dataItem) {
        try {
            List<MeterPoint> list = modelNodeService.getSettingIndex(dataItem.getIndexCode());
            List<String> indexCode = list.stream().map(MeterPoint::getPointId).collect(Collectors.toList());
